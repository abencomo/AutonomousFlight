...
//global Q-position
float global_E, global_N, global_U, global_yaw;
...
void AprilMessageReceived(const apriltags::AprilTagDetections& detectionsMsg) {
	...
	//get new Q-position
	float new_E = 0.0;	float new_N = 0.0;	float new_U = 0.0;	float new_yaw = 0.0;
	for(int i=0; i<mapped.size(); i++)
	{
		...
		new_E = new_E + id_map_x[mapped[i]] + mapped_E;
		new_N = new_N + id_map_y[mapped[i]] + mapped_N;
		new_U = new_U + Q_z_pos[mapped[i]];
		new_yaw = new_yaw + id_map_yaw[mapped[i]] + Q_yaw[mapped[i]];
	}
	new_E = new_E/mapped.size();	new_N = new_N/mapped.size();	new_U = new_U/mapped.size();
	new_yaw = new_yaw/mapped.size();

	if((isnan(new_E) || isnan(new_N) || isnan(new_U) || isnan(new_yaw)))
	{	
		ROS_ERROR("No tags seen!");	
	}
	else
	{
		//no tag seen. give last seen position
		global_E = new_E;	global_N = new_N;	global_U = new_U;	global_yaw = new_yaw;
	}
...
int main(int argc, char **argv)
{
   ...
   ros::NodeHandle n;
   ros::Publisher current_pub = n.advertise<geometry_msgs::PoseStamped>("/mavros/vision_pose/pose",100);
